...
import {DispatcherTravelHistory} from '../../types/dispatcher-travel-history';

const TravelHistoryForDispatchers = () => {
  const [travelData, setTravelData] = useState<DispatcherTravelHistory['data']>(
    [],
  );
  ...
  const [selectedReservations, setSelectedReservations] = useState<
    DispatcherTravelHistory['data'][0]['reservations'] | null
  >(null);

  const fetchTravelHistory = useCallback(async () => {
    setLoading(true);
    try {
      const response = await get(API_ENDPOINTS.GET_DISPATCHERS_TRAVEL_HISTORY);
  ...
  const groupByDate = (data: DispatcherTravelHistory['data']) => {
  ...
  const renderItem = ({item}: any) => (
    <TouchableOpacity
      ...
      <View style={styles.leftSection}>
        ...
        <Text style={styles.referenceNo}>Driver: {item.driver.full_name}</Text>
...
export default TravelHistoryForDispatchers;
